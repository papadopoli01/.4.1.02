...
export default function HeroPage() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const q = query(collection(db, 'hero'), ...constraints);
      const snap = await getDocs(q);
      const data = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      setItems(data);
    } catch (error) {
      toast.error('Erro ao buscar Hero Section');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        await setDoc(doc(db, 'hero', editingItem.id), dataToSave, { merge: true });
        toast.success('Atualizado com sucesso!');
      } else {
        const newRef = doc(collection(db, 'hero'));
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteDoc(doc(db, 'hero', deleteId));
