...
export default function TechnologiesPage() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const q = query(collection(db, 'technologies'), ...constraints);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        await setDoc(doc(db, 'technologies', editingItem.id), dataToSave, { merge: true });
        toast.success('Atualizado com sucesso!');
      } else {
        const newRef = doc(collection(db, 'technologies'));
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteDoc(doc(db, 'technologies', deleteId));
