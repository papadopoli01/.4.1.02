...
collections = [
    ...
    ('technologies', 'Tecnologias', 'name'),
    ('quotes', 'Orçamentos', 'name'),
    ('pages', 'Páginas', 'title'),
    ('blog', 'Blog', 'title'),
    ('hero', 'Hero Section', 'title'),
    ('companies', 'Empresas', 'name'),
    ('users', 'Usuários', 'name')
]
...
export default function COMP_NAMEPage() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const q = query(collection(db, 'COLL_NAME'), ...constraints);
      const snap = await getDocs(q);
      const data = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      setItems(data);
    } catch (error) {
      toast.error('Erro ao buscar TITLE');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        await setDoc(doc(db, 'COLL_NAME', editingItem.id), dataToSave, { merge: true });
        toast.success('Atualizado com sucesso!');
      } else {
        const newRef = doc(collection(db, 'COLL_NAME'));
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteDoc(doc(db, 'COLL_NAME', deleteId));
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {items.map(item => (
              <div key={item.id} className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors">
                <span className="font-medium">{item.MAIN_FIELD}</span>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingItem ? 'Editar' : 'Novo'}>
        <form onSubmit={handleSave} className="space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-medium capitalize">MAIN_FIELD</label>
            <input required value={formData.MAIN_FIELD || ''} onChange={e => setFormData({...formData, MAIN_FIELD: e.target.value})} className="w-full p-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 rounded-lg" />
          </div>
...
for coll, title, main in collections:
    comp_name = coll.capitalize()
    dir_path = f"app/admin/{coll}"
    ...
    if not os.path.exists(file_path):
        ...
        content = content.replace('COLL_NAME', coll)
        content = content.replace('TITLE', title)
        content = content.replace('MAIN_FIELD', main)
